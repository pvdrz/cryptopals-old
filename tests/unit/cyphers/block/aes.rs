...
use cryptopals::cyphers::block::Cypher;
...
fn aes_0() {
    ...
    let cypher = AESCypher::new(key);

    assert_eq!(cyphertext, cypher.encrypt_block(&plaintext));
    assert_eq!(plaintext, cypher.decrypt_block(&cyphertext));
}
